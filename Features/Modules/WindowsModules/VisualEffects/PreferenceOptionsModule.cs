...
namespace GameBoost.Features.Modules.WindowsModules.VisualEffects
{
    public sealed class PreferenceOptionsModule : IInputActionModule<object>, IRecommendedActionModule, IRequireModule
    {
        ...
        {
            var errors = ApplyPerformancePreset();

            if (errors.Count > 0)
                return ModuleResult.Failed(string.Join("\n", errors));

            return ModuleResult.Successful($"Preference options changed to {RecommendedValue}");
        }
        ...
        private static void ApplyRegistryValue(List<string> errors, string path, string key, object value)
        {
