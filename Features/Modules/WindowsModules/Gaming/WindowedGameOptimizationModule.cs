...
namespace GameBoost.Features.Modules.WindowsModules.Gaming
{
    public class WindowedGameOptimizationModule : SystemTweakModuleBase
    {
        ...
        public async override Task<ModuleResult> ExecuteRecommendedAsync(CancellationToken token)
        {
            try
            {
                token.ThrowIfCancellationRequested();

                var newValue = RecommendedValue.Equals(ToggleType.Enabled) ? 1 : 0;
                var result = DirectXUserGlobalHelper.SetDirectXUserGlobalFlag(FLAG_NAME, newValue);

                if (!result.Success)
                    return ModuleResult.Failed(result.Message);

                return ModuleResult.Successful($"Successfully Set {Name} To {RecommendedValue}");
            }
            catch (Exception ex)
            {
#if DEBUG
